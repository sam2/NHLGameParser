...
{
    public static class NHLGameParser
    {
        public static List<NHLGame> GetNHLGames(DateTime date)
        {
            Console.WriteLine("\nParsing BoxScores on " + date.ToShortDateString());
            List<NHLGame> games = new List<NHLGame>();
            List<NHLBoxScore> scores = GetBoxScores(date);
            foreach (NHLBoxScore score in scores)
            {
                ...
                NHLGame game = new NHLGame(score, ParseEvents(pbpURL));
                games.Add(game);
            }
            return games;
        }

        private static List<NHLBoxScore> GetBoxScores(DateTime time)
        {
            string path = "http://live.nhle.com/GameData/GCScoreboard/" + time.ToString("yyyy-MM-dd") + ".jsonp";
            string text = GetPageString(path);
        ...
        private static List<NHLEvent> ParseEvents(string url) //http://www.nhl.com/scores/htmlreports/20152016/PL021201.HTM
        {
